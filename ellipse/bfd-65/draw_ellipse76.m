...
drop=[1,3,6,8,19,21,22,25,26,27,29,32,33,34,39,41,42,44,46,49,56,67,68]%50-55 63
% for i=1:55
for i=1:37
...
%     ezplot(y)
    ...
    B=coefs_pre(i,8)
    C=abs(coefs_pre(i,9))
    ellipse_center_a=center(i,2)
    ellipse_center_b=center(i,3)
    xx=['(x-',num2str(center(i,2)),')'];
    yy=['(y-',num2str(center(i,3)),')'];
    y=[num2str(A),'*',xx,'^2+',num2str(B),'*',xx,'*',yy,'+',num2str(C),'*',yy,'^2=3'];
    cita76(i,:)=[0.5*atan(B/(A-C)),2*(3)/(A+C+sqrt((A-C)*(A-C)+B*B)),2*(3)/(A+C-sqrt((A-C)*(A-C)+B*B))]%A*ellipse_center_a*ellipse_center_a+B*ellipse_center_a*ellipse_center_b+C*ellipse_center_b*ellipse_center_b
    h2=ezplot(y,[-100,100])
    set(h2,'color','black')
%     
%     hold on
...
% p(4)=plot(NaN,NaN);
% set(p(4),'Color','black')
legend(p,'BFD-P','CIELAB','FontSize',11,'Fontname','Helvetica')
